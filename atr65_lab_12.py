...
Original file is located at
    https://colab.research.google.com/drive/1fljh_DBSLpsJXxt1Mi--ATLN5od0WHuk
...
economics_wide = economics_long.pivot(index='date', on='variable', values='value')
...
median_unemploy = economics_wide.filter(pl.col('date') >= '2010-01-01').select(pl.median('unemploy')).item()
...
air_passengers_long = air_passengers.unpivot(index='Year', on=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], variable_name='month', value_name='passengers')
...
std_passengers = air_passengers_long.filter((pl.col('Year') >= 1955) & (pl.col('Year') <= 1960)).select(pl.std('passengers')).item()
...
table2_wide = table2.pivot(index=['country', 'year'], on='type', values='count')
